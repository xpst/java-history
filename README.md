...
This repository pairs **runnable Java code examples** for every release from Java 7 through Java 27 with a **static reference website** summarizing each release's headline features and deprecations.
...
| --- | --- |
| `java7/` … `java27/` | One standalone Maven module per Java release. Each holds 1–3 feature demos plus matching JUnit 5 tests. |
...
| `build.sh` | Convenience wrapper that pins a JDK per module (JDK 25 by default, JDK 26 for `java26/`, JDK 27 for `java27/`) and dispatches to one (or all) of the per-version modules. |
...
| [java27](java27/README.md) | 2026 | Post-quantum TLS, compact object headers by default, G1 everywhere |

## Building & testing

The repo pins **JDK 25** by default (and **JDK 26** for `java26/`, **JDK 27** for `java27/`), plus a project-local `maven-settings.xml`; the `build.sh` wrapper handles both:
...
6. If `<release>N</release>` needs a newer JDK than the default, add a one-line entry to `jdk_for_module()` in `build.sh` (e.g., `java28) echo "/workspace/soft/jdk-28.x" ;;`).
